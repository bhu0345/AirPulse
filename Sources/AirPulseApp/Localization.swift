...
struct L10n {
  ...
  var enableFanControlHint: String {
    t(
      "One-time password to install a system helper — then no more prompts. If the menu closes, click the fan icon in the menu bar again. If you move the app, tap Enable Fan Control again.",
      "只需输入一次密码安装系统助手，之后不再弹窗。若菜单关闭，请再次点击菜单栏风扇图标。若移动了应用位置，请再点一次「启用风扇控制」。"
    )
  }
